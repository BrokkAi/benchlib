...
def test_run_many_tasks_rewrites_success_with_empty_changed_files_to_no_edits(tmp_path: pathlib.Path) -> None:
    project = tmp_path / "repo"
    project.mkdir(parents=True, exist_ok=True)
    ...
    fake_cli = tmp_path / "fake_cli_success_no_edits.py"
    fake_cli.write_text(
        "\n".join(
            [
                "#!/usr/bin/env python3",
    ...
    result_path = results_root / "repo1" / f"dummy-{rev}.json"
    persisted = json.loads(result_path.read_text(encoding="utf-8"))
    assert persisted["stopReason"] == "NO_EDITS"
